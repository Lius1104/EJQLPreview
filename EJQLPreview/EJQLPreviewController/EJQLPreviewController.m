...
@property (nonatomic, strong) UIImage * warningImageBg;
        ...
        if (_fileSize == 0) {
            ...
            [[EJGetURLFileLength defaultFileLength] getUrlFileLength:_Url.absoluteString withResultBlock:^(long long length, NSError *error) {
                if (error) {
                    ...
                    weak_self.fileSize = (NSInteger)length;
                }
...
//    _localPathStr = [file stringByAppendingPathComponent:fileName];
    _localPathStr = [[file stringByAppendingPathComponent:fileName] stringByRemovingPercentEncoding];
    if (isExit) {
...
- (void)handleClickRightItem {
    ...
    _warningView.textColor = [UIColor colorWithRed:34.0/255.0 green:34.0/255.0 blue:34.0/255.0 alpha:1];
    _warningView.separatorInset = UIEdgeInsetsZero;
    _warningView.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
    _warningView.separatorColor = [UIColor colorWithRed:240.0/255.0 green:240.0/255.0 blue:240.0/255.0 alpha:1];
    ...
    _warningView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.5];
    _warningView.showFrom = WarningCardShowFromRightTop;
    
    if (image) {
        _warningView.backgroundImg = image;
    } else {
        _warningView.backgroundImg = self.warningImageBg;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    _localFileUrl = [NSURL fileURLWithPath:_localPathStr];
    NSError * error = nil;
    BOOL result = [[NSFileManager defaultManager] moveItemAtURL:location toURL:_localFileUrl error:&error];
...
//        _progressView.trackTintColor = UIColorHex(E3E3E3);
        _progressView.trackTintColor = [UIColor colorWithRed:227/255.0 green:227.0/255.0 blue:227/255.0 alpha:1];
        if (_progressColor) {
            ...
            _progressView.progressTintColor = [UIColor colorWithRed:98.0/255.0 green:192.0/255.0 blue:187.0/255.0 alpha:1];
        }
...
- (UILabel *)progressLabel {
    if (!_progressLabel) {
        ...
        _progressLabel.textColor = [UIColor colorWithRed:102.0/255.0 green:102.0/255.0 blue:102.0/255.0 alpha:1];
...
- (UIImage *)warningImageBg {
    if (!_warningImageBg) {
        ...
        _warningImageBg = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
    }
    return _warningImageBg;
}
